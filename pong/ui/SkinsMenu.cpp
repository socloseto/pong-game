...
SkinsMenu::SkinsMenu()
  ...
  for (size_t i = 0; i < options.size(); i++) {
    ...
    item.text.setPosition(
        {baseWidth / 2.f, baseHeight * itemsStartYPercent +
                              static_cast<float>(i) * menuItemsOffset});
...
void SkinsMenu::updateLayout(sf::Vector2u windowSize) {
  ...
  for (size_t i = 0; i < items_.size(); i++) {
    items_[i].text.setCharacterSize(
        static_cast<unsigned int>(skinsMenuItemFontSize));
